...
- 显示设备已开启开发者模式，并安装使用 Samsung Partner 证书签名的配套 Tizen 应用

已在 Samsung Smart Monitor M7 / M70B（`LS43BM702UNXZA`，2022 Tizen）上验证。其他型号可能兼容，但不作保证。

## 设置

1. 在显示设备上安装并启动 `tizen/HDMIBrightnessBridge`。
2. 启动 Windows 程序，输入显示设备的局域网 IP。
3. 如需授权，右键托盘图标选择“重新配对电视遥控权限…”，并在显示设备上允许。
4. 左键单击托盘图标即可调节亮度。
