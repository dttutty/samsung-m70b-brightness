(function () {
  ...
  function sameValue(left, right) {
    if (typeof left === 'number' && typeof right === 'number') {
      return isFinite(left) && isFinite(right) && left === right;
    }
    return left === right;
  }
  ...
  function capabilityFor(setting) {
    ...
    // Merely finding a setter is not enough. A setting is advertised writable
    // only after a no-op, same-value write succeeds and reads back unchanged.
    try {
      writeSetting(setting, value);
      var after = readSetting(setting);
      if (!sameValue(value, after)) {
        throw new Error('same-value verification changed the value');
      }
      capability.writable = true;
      capability.writeVerified = 'same-value';
      capability.value = after;
    } catch (writeError) {
      capability.writeError = errorText(writeError);
    }
    return capability;
  }
  ...
  function getCapabilities() {
    ...
    for (var index = 0; index < settings.length; index += 1) {
      ...
      // Experimental picture controls are invisible until both their getter
      // and their same-value setter test have succeeded on this exact firmware.
      if (!settings[index].experimental || capability.writable) {
  ...
  function videoSourceKey(source) {
    if (!source || typeof source.type !== 'string' ||
        typeof source.number !== 'number' || !isFinite(source.number) ||
        Math.round(source.number) !== source.number || source.number < 0) {
      return null;
    }
    return source.type + ':' + source.number;
  }
  ...
  function inputSourceCapability() {
    ...
    try {
      ...
      var currentKey = videoSourceKey(currentSource);
      if (!currentKey) return null;
      addVideoSource(currentSource);
      return {
        id: 'inputSource',
        type: 'enum',
        readable: true,
